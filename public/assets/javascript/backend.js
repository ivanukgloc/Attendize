...
return!1})})};sortable.destroy=function(sortable){_destroySortable(sortable)};sortable.enable=function(sortable){_enableSortable(sortable)};sortable.disable=function(sortable){_disableSortable(sortable)};$.fn.sortable=function(options){return sortable(this,options)};return sortable}));if(typeof jQuery==='undefined'){throw new Error('Bootstrap\'s JavaScript requires jQuery')}+function($){'use strict';function transitionEnd(){var el=document.createElement('bootstrap')
...
Plugin.call($spy,data)})})}(jQuery);(function(factory){if(typeof define==='function'&&define.amd){define(['jquery'],factory)}else{factory((typeof(jQuery)!='undefined')?jQuery:window.Zepto)}}(function($){"use strict";var feature={};feature.fileapi=$("<input type='file'/>").get(0).files!==undefined;feature.formdata=window.FormData!==undefined;var hasProp=!!$.fn.prop;$.fn.attr2=function(){if(!hasProp)
...
this.waitForMove=o.waitForMove||!1
this.clickToClose=o.clickToClose||!1
this.timeoutAfterMove=o.timeoutAfterMove||!1
...
this.removeEventsSet=!0}},_run:function(){if(this._animating||!this.queue.length||!this.el)return
this._animating=!0
...
$(window).resize(function(){rrssbMagicLayout(sizeSmallBtns);waitForFinalEvent(function(){rrssbMagicLayout(sizeSmallBtns)},200,"finished resizing")});rrssbInit()});window.rrssbInit=rrssbInit})(window,jQuery);(function($){'use strict';var _currentSpinnerId=0;function _scopedEventName(name,id){return name+'.touchspin_'+id}
...
else return!1}};(function(factory)
...
{var oDTP=this;oDTP.oData.bArrMatchFormat=[];oDTP.oData.bDateMode=!1;oDTP.oData.bTimeMode=!1;oDTP.oData.bDateTimeMode=!1;var oArrInput=[],iTempIndex;sMode=$.cf._isValid(sMode)?sMode:oDTP.settings.mode;if($.cf._compare(sMode,"date"))
{sFormat=$.cf._isValid(sFormat)?sFormat:oDTP.oData.sDateFormat;oDTP.oData.bDateMode=!0;oArrInput=oDTP.oData.sArrInputDateFormats}
else if($.cf._compare(sMode,"time"))
{sFormat=$.cf._isValid(sFormat)?sFormat:oDTP.oData.sTimeFormat;oDTP.oData.bTimeMode=!0;oArrInput=oDTP.oData.sArrInputTimeFormats}
else if($.cf._compare(sMode,"datetime"))
{sFormat=$.cf._isValid(sFormat)?sFormat:oDTP.oData.sDateTimeFormat;oDTP.oData.bDateTimeMode=!0;oArrInput=oDTP.oData.sArrInputDateTimeFormats}
...
{var oDTP=e.data.obj;oDTP.showDateTimePicker(this);oDTP.oData.bMouseDown=!1},_inputFieldClick:function(e)
...
else{dInput=oDTP.settings.parseDateTimeString.call(oDTP,sDateTime,sMode,$(oInputField))}
...
else{if($.cf._isValid(oDTP.settings.parentElement))
{oArrElements=$(oDTP.settings.parentElement).find("[data-field='date'], [data-field='time'], [data-field='datetime']")}
else{oArrElements=$("[data-field='date'], [data-field='time'], [data-field='datetime']")}}
...
sMode=oDTP.settings.mode;sFormat="Custom";bIs12Hour=!1;if(!oDTP.settings.formatDateTimeString)
...
oDTP._hidePicker(0,oElement);else oDTP._showPicker(oElement)},_setButtonAction:function(bFromTab)
{var oDTP=this;if(oDTP.oData.oInputElement!==null)
...
else oDTP._hidePicker("")}},_setOutput:function(sMode,sFormat,bIs12Hour,dCurrentDate,oElement)
{var oDTP=this;dCurrentDate=$.cf._isValid(dCurrentDate)?dCurrentDate:oDTP.oData.dCurrentDate;bIs12Hour=bIs12Hour||oDTP.oData.bIs12Hour;var oDTV=oDTP._setVariablesForDate(dCurrentDate,!0,!0);var sOutput="",oFDate=oDTP._formatDate(oDTV),oFTime=oDTP._formatTime(oDTV),oFDT=$.extend({},oFDate,oFTime),sDateStr="",sTimeStr="",iArgsLength=Function.length,bAddSeconds;if(oDTP.settings.formatDateTimeString)
{sOutput=oDTP.settings.formatDateTimeString.call(oDTP,oFDT,sMode,oElement)}
else{oDTP._setMatchFormat(iArgsLength,sMode,sFormat);if(oDTP.oData.bDateMode)
...
else{sTimeStr=oFDT.hh+oDTP.settings.timeSeparator+oFDT.mm+oDTP.settings.timeMeridiemSeparator+oFDT.ME}}
else{if(bAddSeconds)
{sTimeStr=oFDT.HH+oDTP.settings.timeSeparator+oFDT.mm+oDTP.settings.timeSeparator+oFDT.ss}
else{sTimeStr=oFDT.HH+oDTP.settings.timeSeparator+oFDT.mm}}
...
sFormat=oDTP.settings.timeFormat;else if($.cf._compare(sMode,"datetime"))
sFormat=oDTP.settings.dateTimeFormat}
oDTP._matchFormat(sMode,sFormat);oDTP.oData.dMinValue=null;oDTP.oData.dMaxValue=null;oDTP.oData.bIs12Hour=!1;var sMin,sMax,sTempDate,dTempDate,sTempTime,dTempTime,sTempDateTime,dTempDateTime;if(oDTP.oData.bDateMode)
...
dTempDate=oDTP.settings.parseDateTimeString.call(oDTP,sTempDate,sMode,$(sStartEndElem));else dTempDate=oDTP._parseDate(sTempDate);if($.cf._compare(sStartEnd,"start"))
{if($.cf._isValid(sMax))
{if(oDTP._compareDates(dTempDate,oDTP.oData.dMaxValue)<0)
oDTP.oData.dMaxValue=new Date(dTempDate)}
else oDTP.oData.dMaxValue=new Date(dTempDate)}
...
else oDTP.oData.dMinValue=new Date(dTempDate)}}}}
if(oDTP.settings.parseDateTimeString)
oDTP.oData.dCurrentDate=oDTP.settings.parseDateTimeString.call(oDTP,sCurrent,sMode,$(oElement));else oDTP.oData.dCurrentDate=oDTP._parseDate(sCurrent);oDTP.oData.dCurrentDate.setHours(0);oDTP.oData.dCurrentDate.setMinutes(0);oDTP.oData.dCurrentDate.setSeconds(0)}
else if(oDTP.oData.bTimeMode)
...
oDTP.oData.dMaxValue=oDTP._parseTime(sMax);if(sStartEnd!==""&&($.cf._compare(sStartEnd,"start")||$.cf._compare(sStartEnd,"end"))&&sStartEndElem!=="")
{if($(sStartEndElem).length>=1)
{sTempTime=oDTP._getValueOfElement($(sStartEndElem));if(sTempTime!=="")
{if(oDTP.settings.parseDateTimeString)
dTempDate=oDTP.settings.parseDateTimeString.call(oDTP,sTempTime,sMode,$(sStartEndElem));else dTempTime=oDTP._parseTime(sTempTime);if($.cf._compare(sStartEnd,"start"))
...
else oDTP.oData.dMaxValue=new Date(dTempTime)}
else if($.cf._compare(sStartEnd,"end"))
...
else oDTP.oData.dMinValue=new Date(dTempTime)}}}}
if(oDTP.settings.parseDateTimeString)
oDTP.oData.dCurrentDate=oDTP.settings.parseDateTimeString.call(oDTP,sCurrent,sMode,$(oElement));else oDTP.oData.dCurrentDate=oDTP._parseTime(sCurrent)}
else if(oDTP.oData.bDateTimeMode)
...
{if(oDTP.settings.parseDateTimeString)
dTempDateTime=oDTP.settings.parseDateTimeString.call(oDTP,sTempDateTime,sMode,$(sStartEndElem));else dTempDateTime=oDTP._parseDateTime(sTempDateTime);if($.cf._compare(sStartEnd,"start"))
{if($.cf._isValid(sMax))
{if(oDTP._compareDateTime(dTempDateTime,oDTP.oData.dMaxValue)<0)
oDTP.oData.dMaxValue=new Date(dTempDateTime)}
else oDTP.oData.dMaxValue=new Date(dTempDateTime)}
else if($.cf._compare(sStartEnd,"end"))
{if($.cf._isValid(sMin))
{if(oDTP._compareDateTime(dTempDateTime,oDTP.oData.dMinValue)>0)
oDTP.oData.dMinValue=new Date(dTempDateTime)}
else oDTP.oData.dMinValue=new Date(dTempDateTime)}}}}
if(oDTP.settings.parseDateTimeString)
oDTP.oData.dCurrentDate=oDTP.settings.parseDateTimeString.call(oDTP,sCurrent,sMode,$(oElement));else oDTP.oData.dCurrentDate=oDTP._parseDateTime(sCurrent)}
...
{iNumberOfColumns=6;sArrFields=["day","month","year","hour","minutes","meridiem"]}}
var sColumnClass="dtpicker-comp"+iNumberOfColumns,bDisplayHeaderCloseButton=!1,bDisplaySetButton=!1,bDisplayClearButton=!1,iTempIndex;for(iTempIndex=0;iTempIndex<oDTP.settings.buttonsToDisplay.length;iTempIndex++)
...
{oDTP._setButtonAction(!0);$("[tabIndex="+(oDTP.oData.iTabIndex+1)+"]").focus();return!1}});$(document).on("keydown.DateTimePicker",function(e)
{if(!$('.dtpicker-compValue').is(':focus')&&parseInt(e.keyCode?e.keyCode:e.which)!==9)
...
{oDTP.oData.bElemFocused=!0;$(this).select()});$('.dtpicker-compValue').blur(function()
{oDTP._getValuesFromInputBoxes();oDTP._setCurrentDate();oDTP.oData.bElemFocused=!1;var $oParentElem=$(this).parent().parent();setTimeout(function()
{if($oParentElem.is(':last-child')&&!oDTP.oData.bElemFocused)
{oDTP._setButtonAction(!1)}},50)});$(".dtpicker-compValue").keyup(function(e)
...
{if(oDTP.settings.buttonClicked)
oDTP.settings.buttonClicked.call(oDTP,"SET",oDTP.oData.oInputElement);oDTP._setButtonAction(!1)});$(oDTP.element).find('.dtpicker-buttonClear').click(function(e)
{if(oDTP.settings.buttonClicked)
...
sElemValue=$(oElem).val();else sElemValue=$(oElem).html();return sElemValue},_setValueOfElement:function(sElemValue,$oElem)
...
sArrDate=sDate.split(oDTP.settings.monthYearSeparator);else sArrDate=sDate.split(oDTP.settings.dateSeparator);if(oDTP.oData.bArrMatchFormat[0])
...
{var oDTP=this;var bIs12Hour=!1,iArgsLength=Function.length;oDTP._setMatchFormat(iArgsLength,sMode,sFormat);if(oDTP.oData.bTimeMode)
...
bSetMeridiem=!0}
else{if(Object.prototype.toString.call(oDTP.oData.dCurrentDate)==='[object Date]'&&isFinite(oDTP.oData.dCurrentDate))
...
if(bValidInput)
return oDTV;else oDTP.oData=$.extend(oDTP.oData,oDTV)},_getValuesFromInputBoxes:function()
...
else{if(oDTP.oData.iCurrentHour>23)
...
{var bCDGTMaxDate=!1,bCDLTMinDate=!1;if(oDTP.oData.dMaxValue!==null)
...
dTempDate=new Date(oDTP.oData.dCurrentDate);else{if(bCDGTMaxDate)
...
$(oDTP.element).find('.month .dtpicker-compValue').val(oFormattedDate.month);else $(oDTP.element).find('.month .dtpicker-compValue').val(oFormattedDate.monthShort)}
else $(oDTP.element).find('.month .dtpicker-compValue').val(oFormattedDate.monthShort);$(oDTP.element).find('.year .dtpicker-compValue').val(oFormattedDate.yyyy);if(oDTP.settings.formatHumanDate)
{oDate=$.extend(oDate,oFormattedDate)}
else{if(oDTP.oData.bDateMode&&(oDTP.oData.bArrMatchFormat[4]||oDTP.oData.bArrMatchFormat[5]||oDTP.oData.bArrMatchFormat[6]))
{if(oDTP.oData.bArrMatchFormat[4])
...
else sDate=oFormattedDate.dayShort+", "+oFormattedDate.month+" "+oFormattedDate.dd+", "+oFormattedDate.yyyy}}
if(oDTP.oData.bTimeMode||oDTP.oData.bDateTimeMode)
...
else{dTempDate=new Date(oDTP.oData.iCurrentYear,oDTP.oData.iCurrentMonth,(oDTP.oData.iCurrentDay+1),oDTP.oData.iCurrentHour,oDTP.oData.iCurrentMinutes,oDTP.oData.iCurrentSeconds,0);if(dTempDate.getTime()>oDTP.oData.dMaxValue.getTime())
...
else{dTempDate=new Date(oDTP.oData.iCurrentYear,oDTP.oData.iCurrentMonth,(oDTP.oData.iCurrentDay-1),oDTP.oData.iCurrentHour,oDTP.oData.iCurrentMinutes,oDTP.oData.iCurrentSeconds,0);if(dTempDate.getTime()<oDTP.oData.dMinValue.getTime())
...
else{if(dTempDate.getTime()>oDTP.oData.dMaxValue.getTime())
$(oDTP.element).find(".meridiem .dtpicker-compButton").removeClass("dtpicker-compButtonEnable").addClass("dtpicker-compButtonDisable")}}
if(oDTP.oData.dMinValue!==null)
{if(oDTP.oData.bTimeMode)
...
$(oDTP.element).find(".meridiem .dtpicker-compButton").removeClass("dtpicker-compButtonEnable").addClass("dtpicker-compButtonDisable")}
else{if(dTempDate.getTime()<oDTP.oData.dMinValue.getTime())
...
else{$(oDTP.element).removeClass("dtpicker-mobile");if(oDTP.oData.oInputElement!==null)
